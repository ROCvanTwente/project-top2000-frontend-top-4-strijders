...
export default function Stijgers() {
  ...
  const [stijgers, setStijgers] = useState([]);
  ...
  useEffect(() => {
    ...
    fetch(`/api/Statistieken/Stijgers?year=${year}`)
      .then(res => {
        if (!res.ok) {
      ...
      .then(data => setStijgers(data))
      .catch(err => console.error(err))
      .finally(() => setLoading(false));
  }, [year]);

  const visibleStijgers = stijgers.slice(0, ITEMS_PER_PAGE);
                ...
                <tr key={song.songId}>
                  ...
                  <td>
                    ...
                    <small className="text-muted ms-2">
                      ({song.positionYearBefore} → {song.position})
                    </small>
                  </td>
                </tr>
              ))}
